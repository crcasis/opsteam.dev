import dashboarddark from '@/assets/img/landing/saas-1/hero/dashboard-dark.jpg'
import dashboardlight from '@/assets/img/landing/saas-1/hero/dashboard-light.jpg'
import { ImgComparisonSlider } from '@img-comparison-slider/react'
import { Col, Container, Image, Row } from 'react-bootstrap'

const Hero = () => {
  return (
    <>
      <section className="position-relative pt-5">
        <div className="position-absolute top-0 start-0 w-100" style={{ height: '79%' }}>
          <div className="bg-primary position-absolute top-0 start-0 w-100 h-100" />
          <div
            className="position-absolute start-0 bottom-0 w-100 overflow-hidden mb-n1"
            style={{ paddingBottom: '6.2%', color: 'var(--ar-body-bg)' }}
          >
            <svg className="position-absolute start-0 bottom-0 w-100 h-100" viewBox="0 0 3000 185.4" xmlns="http://www.w3.org/2000/svg">
              <path fill="currentColor" d="M3000,0v185.4H0V0c496.4,115.6,996.4,173.4,1500,173.4S2503.6,115.6,3000,0z" />
            </svg>
          </div>
        </div>
        <svg
          className="position-absolute end-0 z-2 mt-4 mt-sm-5"
          width={836}
          height={755}
          fill="none"
          stroke="white"
          strokeMiterlimit={10}
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M21.253 754.495c-18.309-66.796-31.642-139.581-6.067-203.929 33.531-84.307 126.54-131.543 216.622-142.348 90.082-10.831 180.803 7.052 271.284 13.945s187.376.931 261.969-50.776C868.476 299.694 902.433 159.981 995.762 75.54 1073.58 5.151 1191.04-18.081 1289.8 17.446M89.774 754.494c-17.324-63.177-29.938-132.049-5.748-192.911 31.722-79.756 119.701-124.438 204.913-134.657 85.239-10.246 171.036 6.68 256.621 13.2s177.263.878 247.812-48.035c97.852-67.835 129.947-199.99 218.248-279.853 73.61-66.584 184.74-88.565 278.15-54.954M158.305 754.496c-16.313-59.585-28.235-124.492-5.402-181.894 29.912-75.206 112.862-117.333 193.231-126.966 80.368-9.661 161.269 6.307 241.983 12.427 80.715 6.148 167.151.825 233.655-45.293 92.264-63.949 122.522-188.574 205.768-263.886 69.4-62.805 174.17-83.509 262.26-51.814M226.834 754.495c-15.328-55.965-26.532-116.96-5.083-170.876 28.103-70.655 106.023-110.228 181.521-119.276 75.499-9.074 151.503 5.908 227.321 11.683s157.011.772 219.497-42.553c86.675-60.09 115.097-177.157 193.31-247.891 65.2-59 163.64-78.453 246.37-48.674M295.359 754.495c-14.344-52.372-24.802-109.429-4.763-159.859 26.266-66.104 99.183-103.122 169.812-111.584 70.628-8.489 141.736 5.535 212.657 10.937s146.899.719 205.366-39.811c81.087-56.205 107.699-165.741 180.859-231.925 60.99-55.193 153.1-73.396 230.48-45.533"
            strokeOpacity=".12"
          />
        </svg>
        <Container className="position-relative z-5 pt-5 mt-4 mt-lg-5" data-bs-theme="dark">
          <Row className="justify-content-center text-center pt-md-2 pt-lg-0 pt-xl-2 pb-5 mb-md-2">
            <Col lg={10} xl={9} xxl={8}>
              <span className="badge bg-light bg-opacity-10 fs-sm text-white px-3 mb-4">Take your data to the next level</span>
              <h1 className="display-3 mb-sm-4">One dashboard to manage all your businesses</h1>
              <p className="text-body pb-3 mb-3 mb-lg-4">Using basic data skills you can improve your business stuff with Around</p>
              <a className="btn btn-lg btn-warning" href="#">
                Start free 14-day trial
              </a>
            </Col>
          </Row>
          <div className="bg-black rounded-4 p-2 mx-auto" style={{ maxWidth: 1160 }}>
            <div className="overflow-hidden" style={{ borderRadius: 'calc(var(--ar-border-radius) * 1.25)' }}>
              <ImgComparisonSlider className="m-n1">
                <figure slot="first">
                  <Image src={dashboarddark} alt="Dashboard Dark Mode" />
                </figure>
                <figure slot="second">
                  <Image src={dashboardlight} alt="Dashboard Light Mode" />
                </figure>
                <div slot="handle" style={{ width: 42 }}>
                  <svg className="text-primary rounded-circle" width={42} height={42} viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
                    <g>
                      <circle fill="currentColor" cx={21} cy={21} r={21} />
                    </g>
                    <path
                      fill="white"
                      d="M25.5019 19.7494H15.9147V15.9146L11.1211 20.7081L15.9147 25.5017V21.6669H25.5019V25.5017L30.2955 20.7081L25.5019 15.9146V19.7494Z"
                    />
                  </svg>
                </div>
              </ImgComparisonSlider>
            </div>
          </div>
        </Container>
      </section>
    </>
  )
}

export default Hero
